...
def show_sizes(TASK,driver):
    ...
    size_btn = None
    try:
        size_btn = driver.find_element_by_xpath("//SPAN[@class='c-form-label-content'][text()='{}T']".format(val))
        size_btn.click()
...
def payment(PROFILE,driver,TASK):
    """
    This module covers all the payment functionalities
    ...
    # Card Details
    try:
        # if this doesn't work, try by id
...
def main_func(TASK,PROFILE,driver):
    """ 
    This module covers the functionality required to checkout for this footlocker.com 
    ...
    # close the initial ad pop up
    try:
        close_btn = WebDriverWait(driver,15).until(EC.presence_of_element_located((By.XPATH, "(//BUTTON[@class='closeButtonGrey lastFocusableElement'])[1]")))
        close_btn.click()
